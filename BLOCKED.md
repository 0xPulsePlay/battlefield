...
## 3. Offline / venue-Wi-Fi CDN risk — RESOLVED (D1, 2026-07-19)
**Was:** `support.js` pulled React + Babel from unpkg and the HUD used Google Fonts → no network
white-screened the page.
**Now:** all three are vendored into `public/vendor/` (React 18.3.1, ReactDOM 18.3.1, Babel-standalone
7.29.0) and the fonts (IBM Plex Mono + Barlow Condensed, latin woff2) into `public/vendor/fonts/`.
No `support.js` edit needed — the DC runtime honours `window.__resources[url]` (a shim in index.html's
`<head>` maps the unpkg URLs to the local copies), and the font `<link>` now points at
`/vendor/fonts.css`. Verified in Playwright with unpkg + fonts.googleapis + fonts.gstatic all blocked:
**zero external requests, app boots, fonts load locally, zero console errors.** Venue Wi-Fi can't
white-screen the demo.

## 3b. Real Solana wallet connect — RESOLVED (C2, 2026-07-19)
**Was:** wallet sign-in created a local guest identity only ("WALLET CONNECT PLANNED").
**Now:** the wallet sheet + home use the Phantom provider directly (`window.phantom.solana`, zero npm
deps — no wallet-adapter bundling needed): `connect()` → pubkey, `signMessage()` sign-in-with-Solana
challenge (no funds, no gas). Predict records key off the pubkey (stored kind = phantom|guest). Graceful
fallback when Phantom isn't installed (GET PHANTOM link + continue-as-guest). DevNet badge retained.
